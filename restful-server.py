...
def predict_type(comments):
    ...
    with graph.as_default():
        types = approval.predict(comments_to_vectors(comments))
    return types[0]

def Classifier():
    approval = Sequential()
    approval.add(Bidirectional(LSTM(64, return_sequences=True),
                            input_shape=(cf.sen_size, cf.word_size)))  # returns a sequence of vectors of dimension 64
    ...
    optimizer = keras.optimizers.RMSprop(lr=0.00005, decay=1e-9)
    approval.compile(loss=keras.losses.categorical_crossentropy,
                  optimizer=optimizer,
                  metrics=['accuracy'])
    approval.load_weights('weights20-filter-lr00005.h5')
    return approval
...
def SAVModel():
    ...
    optimizer = keras.optimizers.RMSprop(lr=0.005, decay=1e-7)
    model.compile(loss=keras.losses.categorical_crossentropy,
                  optimizer=optimizer,
                  metrics=['accuracy'])
    model.load_weights('./weights-rate-lr=0.005-decay=1e-7-epoch50.h5')
    return model
