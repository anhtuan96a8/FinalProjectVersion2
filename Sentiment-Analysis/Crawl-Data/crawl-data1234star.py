...
def crawl_comment_of_item(link_of_item):
    ...
    try:
        ...
        name_element_filter = browser.find_element_by_css_selector("#module_product_review > div > div:nth-child(2) > div > div:nth-child(2)")
        #name_element_filter = browser.find_element_by_xpath("//div[@class = 'pdp-mod-filterSort']/div[@class = 'oper']/span[@class = 'lazada lazada-ic-Filter1 lazada-icon oper-icon']")
        #parent_name_element_filter = name_element_filter.find_element_by_xpath("..")
        print (parent_name_element_filter.text)
        print("oke")

        name_element_filter.click()
        print (name_element_filter.text)
        print ("may dang o cho click nay")
        time.sleep(1)
        name_element_filter_star = browser.find_elements_by_xpath("//div[@class = 'next-overlay-wrapper opened']/div[@class = 'next-menu ver next-overlay-inner animated expandInDown next-position-tl']/ul[@class = 'next-menu-content']/li[@class = 'next-menu-content']")
        print(len(name_element_filter_star))
        print("oke")
        for star in name_element_filter_star:
            print("dang o vong forforfor nay")
            if star.text == "1 star" :
                print("dang o cho ififif nay")
                star.click()
                time.sleep(1)
                while (1):
...
if __name__ == '__main__':
    ...
    for page in range(40):
        ...
        crawl_singer_item("https://www.lazada.vn/products/hinh-that-dong-ho-nu-gedi-quy-phai-full-vien-da-quy-tang-hop-pin-i266166393-s385172615.html?spm=a2o4n.searchlist.list.28.10a25b21tUR2lE&search=1", filename='./lazada-dongho-1strar.csv')
