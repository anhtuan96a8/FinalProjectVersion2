...
def crawl_comment_of_item(link_of_item):
    ...
    comments = []
    rates = []
    #with open('./link-duplicate-lazada-2star.txt') as f:
    #    for line in f:
    #        line = line.rstrip('\n')
    #       if line in link_of_item :
    ...
    try:
        ...
        check_rate = browser.find_element_by_css_selector('#module_product_review > div > div:nth-child(1) > div.mod-rating > div > div > div.detail > ul > li:nth-child(4) > span.percent')
        print(check_rate.text)
        if(check_rate.text != '0'):
            ...
            time.sleep(1)
            try:
                name_element_filter_star = browser.find_element_by_css_selector("body > div:nth-child(41) > div > div > ul > li:nth-child(5)")
                ...
                while (1):
                    ...
                    for cmt in element_cmts:
                        ...
                        if (comment != ""):
                            ...
                            rates.append(2)
                            print("{0} Star: {1}".format(2, comment))
                            pass
                        else:
                            pass
                    # find next button
...
if __name__ == '__main__':
     page = 19
     for page in range(20,60):
         crawl("https://www.lazada.vn/catalog/?from=input&page={0}&q=dong%20ho&rating=1".format(page + 1), filename='./lazada-dongho-2star.csv')
    #crawl_singer_item("https://www.lazada.vn/products/tang-vong-ty-huu-phong-thuy-tang-pin-thay-the-tang-hop-full-box-dong-ho-nam-mat-original-sieu-mong-day-thep-lua-den-cao-cap-dizizid-i256290439-s350944924.html?spm=a2o4n.searchlist.list.6.7eff48600LyCum&search=1", filename='./lazada-dongho-teststar.csv')
